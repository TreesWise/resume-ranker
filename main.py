...
#     criteria_with_weights: List[dict]  # ✅ match frontend
#     uploaded_by: str
#     job_title: str
...
#         input[type="text"], input[type="number"], input[type="file"] { margin-bottom: 10px; }
...
#             <label>Weight:</label>
#             <input type="number" name="weight" value="${weight}" min="0.1" step="0.1" required>
...
#             const weight = parseFloat(item.querySelector('input[name="weight"]').value.trim());
#             if (!criterion || isNaN(weight)) {
#                 resultBox.textContent = "Invalid criterion or weight.";
#                 return;
#             }
#             criteria_with_weights.push({ criterion, weight });
#         }
...
#     // Add one empty criterion on load
